...
def l2_fast_gradient_attack(model, model_lower_bound, model_upper_bound, images,
                            labels, epsilons, random_start):
...
def linf_fast_gradient_attack(model, model_lower_bound, model_upper_bound, images,
                              labels, epsilons, random_start):
    model_bounds = (model_lower_bound, model_upper_bound)
...
def attack_endpoint(request):
    ...
    for norm in norms:
        if norm == '2':
            accuracy = l2_fast_gradient_attack(model, model_lower_bound, model_upper_bound, images, labels, 
                                               epsilons, random_start)
            response_body[norm] = {
            ...
            accuracy = linf_fast_gradient_attack(model, model_lower_bound, model_upper_bound, images, labels,
                                                 epsilons, random_start)
            response_body[norm] = {
                'epsilons': epsilons.tolist(),
                'accuracy': accuracy.tolist()
