...
def attack_endpoint(request):
    ...
    for norm in norms:
        if norm == '2':
            epsilons, accuracy = l2_fast_gradient_attack(model, model_lower_bound, model_upper_bound,
            ...
            epsilons, accuracy = linf_fast_gradient_attack(model, model_lower_bound, model_upper_bound,
                                                           images, labels, epsilon_max, epsilon_num, random_start)
            response_body[norm] = {
                'epsilons': epsilons.tolist(),
                'accuracy': accuracy.tolist()
