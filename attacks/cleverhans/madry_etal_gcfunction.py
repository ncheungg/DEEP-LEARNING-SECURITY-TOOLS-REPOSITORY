...
def cleverhans_mea_func(request):
    ...
    for eps in epsilons:
        ...
        for i in range(len(images)):
            ...
            x_mea = madry_et_al(model_fn = model, x = x, eps = eps, eps_iter = request_json["epsilon_iter"], nb_iter = request_json["attack_iter"], norm = order_of_norm)
            y_pred_mea = model(x_mea)
            test_acc_mea(y, y_pred_mea)

        accuracy_list.append(round(float(test_acc_mea.result()) * 100, 2))
